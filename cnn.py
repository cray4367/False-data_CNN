...
def build_model(input_shape):
    model = keras.Sequential([
        ...
        layers.Dense(1, activation='sigmoid')  # Output: 0 (real) or 1 (false data)
    ])
...
def save_report(accuracy, precision, recall, f1, conformity_scores):
    report = pd.DataFrame({
...
def train_and_evaluate(file_path):
    ...
    save_report(accuracy, precision, recall, f1, conformity_scores)
    
    plot_training_history(history)
    plot_conformity_distribution(conformity_scores)
    return model

# Function for continuous live input from the user
...
file_path = 'your_dataset.csv'  # Update with actual file path
model = train_and_evaluate(file_path)

# Start live input detection
